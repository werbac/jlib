...
#using DataStructures, DataFrames, DataFramesMeta, StatsFuns, GLM, JuMP, NLopt, HDF5, JLD, Distributions, MixedModels, RCall, StatsBase, xCommon
#using StatStack
using DataFrames
...
Create a DataFrame from the `csv_final` file taking column names from the `Headers` file
"""
function loadDF(dir::String)
    isdir(dir) || throw(ArgumentError("\"$dir\" is not a directory"))
    fnms = readdir(dir)
    headnm = filter(r"^Headers", fnms)
    if length(headnm) != 1
        throw(ArgumentError("\"$dir\" should contain exactly one file starting with \"Headers\""))
    end
    dfrm = readtable(joinpath(dir, filter(r"^final_csv", fnms)[1]), header = false,
        separator = ',',  names = Symbol.(lowercase.(chomp.(readlines(joinpath(dir, headnm[1]))))),
        makefactors = true)
    dfrm = dfrm[convert(Vector{Bool}, map(x -> length(unique(x)) > 1, dfrm.columns))]
#    map!(x -> (isa(x, PooledDataArray) && !anynull(x)) ? pool(map(e -> e.value, x)) : x,
#        dfrm.columns)
    map!(x -> (anyna(x) || isa(x, PooledDataArray)) ? x : Array(x), dfrm.columns)
    dfrm
end

"""
    isValid(df_data::DataFrame, cfg::OrderedDict)

Predicate to check `df_data` against `cfg` for validity
"""
function isValid(df_data::DataFrame, cfg::Dict)
    dfnames = names(df_data)
    if !all(x -> x in dfnames, cfg[:all_mandatory_vars])
        throw(ArgumentError("Not all cfg[:all_mandatory_vars] in dataset"))
    end
    if !all(x -> x in dfnames, cfg[:scoring_vars])
        throw(ArgumentError("Not all cfg[:scoring_vars] in dataset"))
    end
    true
end

Base.lowercase(v::Vector{Symbol}) = Symbol.(lowercase.(string.(v)))
Base.filter(r::Regex, v::Vector{Symbol}) = Symbol.(filter(r, string.(v)))
"""
    getCFG(df_in::DataFrame, basedir::String)

Create the configuration Dict from defaults and the `app.json` file in `basedir`
"""
function getCFG(df_in::DataFrame, basedir::String)
    cfg = Dict(
        :P2_Competitor => true,
        ...
        :sigLevel => "0.2",  # why a string and not a number.  It is easier to convert a number to a string than vice versa.
        ...
        :scoring_vars => lowercase([:Prd_1_Net_Pr_PRE, :Prd_1_Net_Pr_POS, :Buyer_Pos_P0, :Buyer_Pre_P0]),
        :all_mandatory_vars => lowercase([:Buyer_Pos_P1, :Buyer_Pre_P1, :Trps_POS_P1, :Trps_PRE_P1,
            :Dol_per_Trip_POS_P1, :Dol_per_Trip_PRE_P1, :Nonbuyer_Pre_P1,
            :hh_age, :estimated_hh_income, :number_of_children_in_living_Un, :person_1_gender, :experian_id]),
        )
    override = joinpath(basedir, "app.json")
    if isfile(override)
        for (k, v) in JSON.parsefile(override)
            ksym = Symbol(k)
            if haskey(cfg, ksym)
                current = cfg[ksym]
                if isa(current, Dict{Symbol, Any})
                    cfg[ksym] = union(current, convert(Vector{Symbol}, v))
                else
                    cfg[ksym] = v
                end
            else
    ...
    cfg[:proscore] = filter(r"model", names(df_in))   # Set the ProScore variable in the dataset   #push!(cfg[:all_mandatory_vars], cfg[:ProScore])
    isempty(cfg[:proscore]) && throw(ArgumentError("Missing response variable in data"))
    cfg[:num_products] = length(filter(r"^buyer_pre_p", names(df_in))) - 1  # get number of products in the data
    cfg[:random_campaigns] = intersect(cfg[:random_campaigns], names(df_in))
    cfg
end

function reworkCFG!(df_in::DataFrame, cfg::Dict)
    dnames = names(df_in)
    xflags = [cfg[:exposed_flag_var], :exposed_flag]
    features = setdiff(dnames, xflags)

    cfg[:xVarsDemos] = vcat([:experian_id, :banner, :prd_0_qty_pre],
        features[findfirst(features, :state):findfirst(features, :mosaic)])   # exclude demos between....
