...
function data_Prep(dfd::DataFrame, cfg::OrderedDict)
    ...
    for c in setdiff(vars[findin(vars[:eltypes],[String]),:names],cfg[:allrandoms]) # set variables as.numeric and replace NA's with zero
        ...
        try dfd[c] = map( x -> DataFrame.isna.(x) ?  NaN : convert(Float64, x)  , dfd[c]) catch e println("  (failed)") end  #NA to NaN
...
type xGLM
    ...
    function xGLM(dfd::DataFrame, dist::Distribution, y_var::Symbol, lnk::Link , vfactors::Array{Symbol} )  
        this=new()
        this.wasSuccessful=false
        this.xvars=Symbol[]
        #this.vfactors=setdiff(vfactors, [y_var,logvar])
        this.vfactors=vfactors
        for l in 1:30
            #log_colname=Symbol("LOG_"*string(logvar))
            #this.vfactors=vcat(setdiff(this.vfactors,this.xvars),[log_colname])
            this.fmula = genFmula(y_var, this.vfactors )
            try
...
function getOutliers(m::MModel,segment::Symbol,pct::Int64) # :data_nb_ne_b, :data_b_e_nb, :pen_reduction, :occ_reduction, :dolocc_reduction
